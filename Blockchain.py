...
# Импорт базы данных
def connect_db():
    conn = psycopg2.connect(dbname='postgres', user='postgres', password='1111', host='95.165.31.95', port='25432')
    cursor = conn.cursor()
...
# Эта функция ниже создана для добавления дополнительных блоков в цепочку
    def create_block(self, proof, hashed_data):
        block = {
            ...
            'passendger_name': str(database[1].iloc[len(self.chain)]),
            'flight': str(database[2].iloc[len(self.chain)]),
            'passport':str(database[3].iloc[len(self.chain)]),
            'departue':str(database[4].iloc[len(self.chain)]),
            'destination':str(database[5].iloc[len(self.chain)]),
            'luggage_id':str(database[6].iloc[len(self.chain)]),
...
def mine_block():
    ...
    response = {'message': 'A block is MINED',
                'index': block['index'],
                'id': block['id'],
                'passendger_name': block['passendger_name'],
                'flight': block['flight'],
                'passport':block['passport'],
                'departue':block['departue'],
                'destination':block['destination'],
                'luggage_id':block['luggage_id'],
